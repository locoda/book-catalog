...
- **年份**：初版年。
- **题名词**：slug 中第一个非停用词的段，大写，取前 4 个字符（不足 4 用全部）。停用词表（封闭）：`a` `an` `the`。
  - 例：`hisoyaka-na-kessho` → `HISO`；`the-long-game` → `LONG`（跳过 the）；`5-de-gai-bian` → `5`；`ai-lian` → `AI`。
...
- 词表变更只能由馆长（Linda）批准；批准后在 `src/data/subjects.yaml` 与本节同步增删，并在 §8 版本记录里记一行。
- **机器可读单一来源是 `src/data/subjects.yaml`**——schema（`src/content.config.ts`）和 `scripts/validate.mjs` 都从它读取；不在词表内的主题词会阻断构建。本节列表为人读镜像。
