...
**按 CATALOGING.md §0.4 的规则，`readings` 是"事实数据，原样保留，不修改不删除"，所以本轮没有动这些字段，只做记录。** 但这意味着：目前网站上凡是依赖 readings 日期算出来的信息（作者页"阅读 20xx–20xx"统计、按年份排序等）在这 160 条记录上都是不可信的。README 里说"标了 `placeholder: true` 的条目阅读事件是示意数据"，但这 160 条里没有一条实际标了 `placeholder: true`——也就是说占位数据没有被正确标记，混进了"事实数据"里。
...
### 日本語原書直読（mine 言語閉鎖集例外）

- [sarome-no-dantoudai] 『サロメの断頭台』2024：馆长于 2026-09-01/02 以日文原版电子书读完，NeoDB 标记 2026-09-02 评 8 分，评语“怪东西！但蛮好看的哈哈哈”。该书尚无中/英译本，按 CATALOGING.md §2，`mine: true` 只能落在 zh-Hans/zh-Hant/en，ja 直读无法标 mine，暂不设 mine，显示层将以原题作主标题。StoryGraph 完成日 2026-09-01 与 NeoDB 2026-09-02 差 1 天为时区换算所致（PT 9/1 22:53 = UTC 9/2 05:53），非事实差异。
