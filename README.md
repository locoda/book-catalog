...
| `docs/CONFIRM-PLAYBOOK.md` | 确认作业手册——AI 逐批联网核实 → 馆长批准 → `confirmed: true` 冻结 |
| `docs/QUESTIONS.md` | 编目疑点追踪——词表提案、数据异常、待馆长确认事项 |

## 下一步

v1 主体工程已完成——493 条作品全部导入并著录，网站上线运行。后续围绕三个方向渐进迭代：

**数据质量**：逐条联网核实（callno 分类、原语言、初版年、著者归属），逐步推进 `confirmed: true` 冻结。这是目前最主要的工作线。

**数据富化**：微信读书笔记/高亮回填、豆瓣豆坟 CSV 导入——把散落在各平台上的阅读痕迹回拢到目录里，让每条记录的厚度不只是编目元数据。

**站点打磨**：主题词表详情页（词 → 定义 → 馆藏统计）、可能的检索/筛选功能。
